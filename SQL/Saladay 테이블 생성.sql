...
COMMENT ON COLUMN "OPTION"."CALORIE" IS '옵션 칼로리';
...
COMMENT ON COLUMN "MENU"."CALORIE" IS '메뉴 칼로리';
...
COMMENT ON COLUMN "ORDER_MENU"."ORDER_MENU_PRICE" IS '주문한 메뉴 가격';
...
COMMENT ON COLUMN "ORDER"."ORDER_DEL_FL" IS '주문 취소 여부(Y:취소, N:주문, A:관리자취소)';
...
CREATE TABLE "ORDER_MENU_OPTION" (
	...
	"ORDER_OPTION_PRICE"  NUMBER NOT NULL
);
...
COMMENT ON COLUMN "ORDER_MENU_OPTION"."ORDER_OPTION_PRICE" IS '주문한 옵션 가격';
...
CREATE SEQUENCE SEQ_DELIVERY_NO NOCACHE; -- 배송번호
...
CREATE SEQUENCE SEQ_FAQ_NO NOCACHE;  --FAQ 번호

COMMIT;

-- 장바구니 삭제 관련

-- CART_MENU_OPTION테이블의 CART_MENU_NO 외래키
-- delete cascade 제약조건이 없는 외래키 삭제하기
ALTER TABLE SALADAY.CART_MENU_OPTION DROP CONSTRAINT FK_CART_MENU_TO_CART_MENU_OPTION_1;

-- delete cacade 제약조건 외래키 다시 추가하기
ALTER TABLE SALADAY.CART_MENU_OPTION ADD CONSTRAINT FK_CART_MENU FOREIGN KEY (CART_MENU_NO) REFERENCES SALADAY.CART_MENU(CART_MENU_NO) ON DELETE CASCADE;

-- CART_MENU 테이블의 CART_NO 외래키
-- delete cascade 제약조건이 없는 외래키 삭제하기
ALTER TABLE SALADAY.CART_MENU DROP CONSTRAINT FK_CART_TO_CART_MENU_1;
-- delete cacade 제약조건 외래키 다시 추가하기
ALTER TABLE SALADAY.CART_MENU ADD CONSTRAINT FK_CART FOREIGN KEY (CART_NO) REFERENCES SALADAY.CART(CART_NO) ON DELETE CASCADE;
...
COMMENT ON COLUMN "FAQ"."FAQ_NO" IS '자주 묻는 질문 번호(SEQ_FAQ_NO)';
COMMENT ON COLUMN "FAQ"."FAQ_TITLE" IS 'FAQ제목';
COMMENT ON COLUMN "FAQ"."FAQ_CONTENT"IS 'FAQ내용';
COMMENT ON COLUMN "FAQ"."FAQ_DEL_FL" IS 'FAQ삭제여부(Y: 삭제)';
