...
-- 당월 
SELECT TO_CHAR(ADD_MONTHS(SYSDATE,0),'YY/MM') FROM DUAL;
-- 다음달
...
-- 메뉴 별 당월 판매 수량
SELECT MENU_NO ,(SELECT TO_CHAR(ADD_MONTHS(SYSDATE,0),'YY/MM') FROM DUAL) AS "MONTH" , COUNT(*) AS MONTH_MENU_COUNT
FROM ORDER_MENU
JOIN "ORDER" USING (ORDER_NO)
JOIN MENU USING(MENU_NO)
WHERE (SELECT TO_CHAR(ADD_MONTHS(SYSDATE,0),'YY/MM') FROM DUAL)=SUBSTR(ORDER_DATE,1,5)
GROUP BY 2 , MENU_NO
ORDER BY MENU_NO ASC;

-- 메뉴 별 당월 매출액
SELECT MENU_NO, (SELECT TO_CHAR(ADD_MONTHS(SYSDATE,0),'YY/MM') FROM DUAL) AS "MONTH" , SUM(MENU_PRICE) AS MONTH_MENU_PRICE
FROM ORDER_MENU
JOIN "ORDER" USING (ORDER_NO)
JOIN MENU USING(MENU_NO)
WHERE (SELECT TO_CHAR(ADD_MONTHS(SYSDATE,0),'YY/MM') FROM DUAL)=SUBSTR(ORDER_DATE,1,5)
...
-- 메뉴 별 전월 매출액
SELECT MENU_NO, (SELECT TO_CHAR(ADD_MONTHS(SYSDATE,-1),'YY/MM') FROM DUAL) AS "MONTH" , SUM(MENU_PRICE) AS MONTH_MENU_PRICE
FROM ORDER_MENU
JOIN "ORDER" USING (ORDER_NO)
JOIN MENU USING(MENU_NO)
WHERE (SELECT TO_CHAR(ADD_MONTHS(SYSDATE,-1),'YY/MM') FROM DUAL)=SUBSTR(ORDER_DATE,1,5)
GROUP BY 2, MENU_NO
ORDER BY MENU_NO ASC;
